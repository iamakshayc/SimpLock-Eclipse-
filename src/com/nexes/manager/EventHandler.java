...
public class EventHandler implements OnClickListener {
	...
	public EventHandler(Context context, final FileManager manager) {
		mContext = context;
		mFileMang = manager;
		password=mContext.getSharedPreferences("simplock",Context.MODE_WORLD_READABLE);
		mDataSource = new ArrayList<String>(mFileMang.setHomeDir
						(Environment.getExternalStorageDirectory().getPath()+File.separator+".simplock"));
	}
	...
	public void onClick(View v) {
		...
		switch(v.getId()) {
			...
			case R.id.back_button:			
				if (mFileMang.getCurrentDir() != password.getString("location","/sdcard/.simplock")) {
					Log.v("current",mFileMang.getCurrentDir());
		...
		/*	case R.id.home_button:		
				...
				stopThumbnailThread();
				updateDirectory(mFileMang.setHomeDir("/sdcard/.simplock"));
				if(mPathLabel != null)
					mPathLabel.setText(mFileMang.getCurrentDir());
				break;
			...
			case R.id.lock_button:
				...
				{
					mDelegate.addalllist(i,mFileMang.getCurrentDir());
				}
			...
			case R.id.hidden_attach:
    			...
    			for(int i = 0; i < length; i++) {
    				file = new File(mMultiSelectData.get(i));
    			...
    			mDelegate.killMultiSelect(true);
				break;
			case R.id.hidden_lock:
				if(mMultiSelectData == null || mMultiSelectData.isEmpty()) {
					mDelegate.killMultiSelect(true);
					break;
				}
				length = mMultiSelectData.size();
				for(int i = 0; i < length; i++) {
    				File from = new File(mMultiSelectData.get(i));
    				if(!getfiletype(from).toLowerCase().contains("smp")&&!getfiletype(from).toLowerCase().contains("nomedia")&&!from.isDirectory())
    				{
    ...
    public class TableRow extends ArrayAdapter<String> {
    	...
    	public void addMultiPosition(int index, String path) {
    		...
    		//Toast.makeText(mContext,positions,Toast.LENGTH_SHORT).show();
    		notifyDataSetChanged();
    	}
    	public void addalllist(int index, String path) {
    		if(positions == null)
    		{
    			positions = new ArrayList<Integer>();
